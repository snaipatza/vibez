...
function LivePage({
  ...
  // Load ads + refresh every 2 minutes
  useEffect(() => {
    const loadAds = () => fetch('/api/ads').then(r => r.json()).then(d => { if (Array.isArray(d)) setAds(d); }).catch(() => {});
    loadAds();
    const id = setInterval(loadAds, 120000);
    return () => clearInterval(id);
  }, []);
...
function ChatPanelInline({ messages, onSend, user, soundOn = true, typeSoundOn = true, socket, activeRoom }) {
  ...
  useEffect(() => {
    ...
    const handler = ({ room, users }) => {
      ...
      // server already excludes the sender, just filter any stale self-entry by username
      setTypingUsers((users || []).filter(u => u.username !== user.name).map(u => u.display_name || u.username));
    };
  ...
  const handleTextChange = (val) => {
    ...
    if (val.trim()) {
      if (!isTypingRef.current) {
        ...
        socket.emit('chat:typing', { room: activeRoom, username: user.name, display_name: user.display_name || user.name });
      }
